...
def main():
    ...
    # Added optional argument for independent vertex noise range
    parser.add_argument(
        "--noise", "-n",
        type=float,
        default=1.0,
        help="Simulated independent coordinate noise range applied to L2 vertices (+/- meters). Default is 1.0"
    )
    ...
    # =========================================================================
    print(f"[PROCESS] Pass 3: Applying custom [FIX_PARCEL] spatial shifts and class updates (Noise: +/-{args.noise}m)...")
    ideal_parcels = []
    measured_parcels = []
    ids = []
    classes = []
    ...
    for record in raw_records:
        ...
        if p_id_str in fix_parcels:
            ...
            # Inject optional independent uniform random noise onto L2 polygon vertices
            if active_class == "L2" and args.noise > 0:
                coords = np.array(poly_ideal.exterior.coords)
                noisy_coords = []
                for i in range(len(coords) - 1):
                    noise_x = np.random.uniform(-args.noise, args.noise)
                    noise_y = np.random.uniform(-args.noise, args.noise)
                    noisy_coords.append((coords[i][0] + noise_x, coords[i][1] + noise_y))
                noisy_coords.append(noisy_coords[0])
                poly_measured = Polygon(noisy_coords)
            else:
        ...
        vertex_sequences.append(record["Vertex_Sequenc"])
    ...
    # Refactored to pass the distorted measured fabric to PlotMap for stage 1
    map_layers = {
        "Warped Fabric Output": gdf_measured,
        #"L1 Framework": gdf_ideal,
    }
    ...
    PlotMap.render_map(
        layers=map_layers,
        title="Simulation Workspace Input (Distorted Measured Fabric & Locked Node Structures)",
